...
// convars.
daynight.dayspeed = CreateConVar( 'daytime_speed', '30', { FCVAR_REPLICATED , FCVAR_ARCHIVE , FCVAR_NOTIFY } )
...
function daynight:init( )
	...
	if( daynight.ClockMins == nil ) then
		daynight.ClockMins = 1
	end
...
function daynight:buildLightTable( )
	/*
	[ [ [ NOTE ] ] ]
...
// environment think.
function daynight:think( )

	if daynight.ClockStarted then
		...
		local daylen = daynight.dayspeed:GetFloat( )
		
		// delay next think.
		self.nextthink = CurTime( ) + ( ( daylen / 1440 ) * 60 )
		
		// progress the time.
		daynight.ClockMins = daynight.ClockMins + 1
		
		// Clock
		if ( daynight.ClockMins > DAY_LENGTH ) then
			daynight.ClockMins = 1
			daynight.ClockDay = daynight.ClockDay + 1
		end
		
		if ( daynight.ClockDay >= get_days_in_month( daynight.ClockMonth, daynight.ClockYear ) ) then 
			daynight.ClockMonth = daynight.ClockMonth + 1
			daynight.ClockDay = 1
		end
		if ( daynight.ClockMonth > 12 ) then 
			daynight.ClockYear = daynight.ClockYear + 1
			daynight.ClockMonth = 1
		end
		
		daynight.ClockTime = DAY_START + daynight.ClockMins - 1
		
		// light pattern.
		local pattern = self.light_table[ daynight.ClockMins ].pattern
		...
		local sky_overlay_alpha = self.light_table[ daynight.ClockMins ].sky_overlay_alpha
		local sky_overlay_color = self.light_table[ daynight.ClockMins ].sky_overlay_color
		if ( self.sky_overlay ) then
			local brush
			for _ , brush in pairs( self.sky_overlay ) do
		...
		local env_sun_angle = self.light_table[ daynight.ClockMins ].env_sun_angle
		local shadow_angles = self.light_table[ daynight.ClockMins ].shadow_angle
	...
	function daynight:PushDayEffects( isDay )
		if (isDay == daynight.DayEffects) then return false end
		
		daynight.DayEffects = isDay
...
// add night lights.
function daynight:checkNightLight( ent , key , val )
...
hook.Add( 'InitPostEntity' , 'DayNight_Init' , function( ) daynight:init( ) end )
hook.Add( 'Think' , 'DayNight_Think' , function( ) daynight:think( ) end )
hook.Add( 'EntityKeyValue' , 'DayNight_CheckNightLight' , function( ent , key , val ) daynight:checkNightLight( ent , key , val ) end )
