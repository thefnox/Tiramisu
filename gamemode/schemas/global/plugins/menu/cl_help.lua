...
local abouttext = [[From TIRAscript to Tiramisu:

Tiramisu ( which is, btw, a fancy TIRA. Pun intended. ) is based off TIRAscript G2 ( Thank Luabanana for creating it! ), only modified extensively, to the point that only around 15% of the base code remains totally untouched. There are some notable changes that you should keep in mind while testing features or while editing code, to name a few:

-The animation system required a complete revamp of the way that GMod regularily handles player models. Scripts made for TIRAscript G2 that are meant to change a player's model probably don't work anymore.
...
-Absolutely all VGUI elements have been revamped. We have managed to retain TIRAscript's simplicity while adding functionality.
...
function OpenHelp()
	...
	propsheet:AddSheet("About", apanel, "gui/silkicons/magnifier", false, false, "About Tiramisu and TIRAscript");
...
TIRA.RegisterMenuTab( "Help", OpenHelp, CloseHelp )