...
function CAKE.HandleGear( ply, item, bone, itemid, offset, angle, scale, skin )
	...
	if CAKE.ItemData[ item ] then
		...
		if CAKE.ItemData[ item ].BoneMerge then
			bonemerge = true
		end
		...
		if IsValid( ply.Gear[ id ]:GetPhysicsObject( ) ) then
...
function CAKE.RemoveGearItem( ply, item )
	...
	if ply.Gear then
		for k, v in pairs( ply.Gear ) do
			if IsValid( v ) and v.item == item then
				CAKE.RemoveGear( ply, k )
				CAKE.SaveGear( ply )
				return true
			end
...
net.Receive( "Tiramisu.GetEditGear", function(len, ply)
	...
	if IsValid( ent ) and ent:GetDTEntity( 1 ) == ply then
...
function meta:HideActiveWeapon()
	...
	if IsValid( self ) then
		...
		if IsValid( wep ) and !self:GetNWBool( "observe" ) then
